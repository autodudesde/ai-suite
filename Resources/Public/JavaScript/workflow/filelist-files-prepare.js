...
class FilelistFilesPrepare {
    ...
    async sendFilesToUpdate(formData) {
        Generation.showSpinner();
        let res = await Ajax.sendAjaxRequest('aisuite_workflow_filelist_files_update', formData);
        Generation.hideSpinner();
        return General.isUsable(res)
    }
    async sendFilesToExecute(formData, selectedFiles, handledFiles, maxRetries = 2, delay = 1000) {
        try {
            if (!formData.has('workflowFilesExecute[files]')) {
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                try {
                    let res = await Ajax.sendAjaxRequest('aisuite_workflow_filelist_files_execute', formData);
    ...
    createFormData(baseFormData, currentFiles) {
        ...
        formData.append('workflowFilesExecute[parentUuid]', baseFormData.parentUuid);
        formData.append('workflowFilesExecute[column]', baseFormData.column);

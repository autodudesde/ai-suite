...
class FileReferencePrepare {
    ...
    init() {
        let startFromPidInput = document.querySelector('input[name="workflowFileReferencesPrepare[startFromPid]"]');
                ...
                if(ev && ev.target) {
                    ...
                    if(ev.target.nodeName === 'BUTTON' && ev.target.type === 'submit' && ev.target.id === 'fileReferencesSaveMetadataSubmitBtn') {
                        ...
                        if(Object.keys(selectedFileReferences).length === 0) {
                            Notification.warning(TYPO3.lang['aiSuite.notification.generation.workflow.missingSelection'], TYPO3.lang['aiSuite.notification.generation.workflow.missingFiles']);
                        } else {
                            let formData = new FormData();
                            formData.append('workflowFileReferencesExecute[column]', document.querySelector('select[name="workflowFileReferencesPrepare[column]"]').value);
                            formData.append('workflowFileReferencesExecute[sysLanguage]', document.querySelector('select[name="workflowFileReferencesPrepare[sysLanguage]"]').value);
                            formData.append('workflowFileReferencesExecute[fileReferences]', JSON.stringify(selectedFileReferences));
                    ...
                    if(ev.target.nodeName === 'BUTTON' && ev.target.type === 'submit' && (ev.target.id === 'fileReferencesExecuteFormSubmitBtn' || ev.target.id === 'automaticFileReferencesExecuteFormSubmitBtn')) {
                        ...
                        if(Object.keys(selectedFileReferences).length === 0) {
                            ...
                            const baseFormData = {
                                ...
                                column: document.querySelector('select[name="workflowFileReferencesPrepare[column]"]').value,
    ...
    async prepareFileReferences(formData) {
        let res = await Ajax.sendAjaxRequest('aisuite_workflow_filereferences_prepare', formData);
        if (General.isUsable(res)) {
            if(General.isUsable(res.output) && !General.isUsable(res.output.content)) {
                ...
                const languageSelect = document.querySelector('select[name="workflowFileReferencesPrepare[sysLanguage]"]');
            ...
            LibrarySelection.initialize(
                ...
                ['#fileReferencesExecuteFormSubmitBtn', '#automaticFileReferencesExecuteFormSubmitBtn']
            );
        }
    }
    ...
    async sendFileReferencesToExecute(formData, selectedFileReferences, handledFileReferences, maxRetries = 2, delay = 1000) {
        try {
            if (!formData.has('workflowFileReferencesExecute[fileReferences]')) {
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                try {
                    let res = await Ajax.sendAjaxRequest('aisuite_workflow_filereferences_execute', formData);
    ...
    async sendFileReferencesToUpdate(formData) {
        let res = await Ajax.sendAjaxRequest('aisuite_workflow_filereferences_update', formData);
        if (General.isUsable(res)) {
            Generation.hideSpinner();
    ...
    createFormData(baseFormData, currentFileReferences) {
        let formData = new FormData();
        formData.append('workflowFileReferencesExecute[parentUuid]', baseFormData.parentUuid);
        formData.append('workflowFileReferencesExecute[column]', baseFormData.column);
        formData.append('workflowFileReferencesExecute[sysLanguage]', baseFormData.sysLanguage);
        ...
        formData.append('workflowFileReferencesExecute[customPrompt]', baseFormData.customPrompt ?? '');

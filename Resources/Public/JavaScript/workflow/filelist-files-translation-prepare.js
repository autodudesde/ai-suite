...
class FilelistFilesTranslationPrepare {
    ...
    async sendFilesToExecute(formData, selectedFiles, handledFiles, maxRetries = 2, delay = 1000) {
        try {
            if (!formData.has('workflowFilesTranslationExecute[files]')) {
            ...
            for (let attempt = 1; attempt <= maxRetries; attempt++) {
                try {
                    let res = await Ajax.sendAjaxRequest('aisuite_workflow_filelist_files_translate_execute', formData);
    ...
    createFormData(baseFormData, currentFiles) {
        ...
        formData.append('workflowFilesTranslationExecute[parentUuid]', baseFormData.parentUuid);
        formData.append('workflowFilesTranslationExecute[column]', baseFormData.column);
        formData.append('workflowFilesTranslationExecute[sourceLanguage]', baseFormData.sourceLanguage);
        formData.append('workflowFilesTranslationExecute[targetLanguage]', baseFormData.targetLanguage);
    ...
    async updateContent() {
        ...
        let res = await Ajax.sendAjaxRequest('aisuite_workflow_filelist_files_translate_update_view', formData);
        if (General.isUsable(res)) {
            ...
            LibrarySelection.initialize(
                ...
                ['#filesTranslationExecuteFormSubmitBtn', '#automaticFilesTranslationExecuteFormSubmitBtn']
            );
        }
        await this.handleTextAiModelChange('ChatGPT');
        Generation.hideSpinner();

...
class AbstractBackendController
{
    ...
    {
        // site parameter is not included when accessing filelist module, so we need a workaround here
        if($this->request->getAttribute('site') !== null && $this->request->getAttribute('site')->getRootPageId() > 0) {
            $rootPageId = $this->request->getAttribute('site')->getRootPageId();
        } else {
            $rootPageId = $this->siteService->getIdOfFirstRootPage();
        }
        $uriParameters = [
            'id' => isset($this->request->getQueryParams()['id']) && !str_starts_with($this->request->getAttribute('route')->getPath(), '/module/file/AiSuite') ? $this->request->getQueryParams()['id'] : $rootPageId,
            'action_route' => $route,
        ];
        if(str_starts_with($this->request->getAttribute('route')->getPath(), '/module/file/AiSuite')) {
            $route = 'web_AiSuite';
        }
    ...
    protected function renderView(string $template, array $assignMultiple): string
    {
        $view = GeneralUtility::makeInstance(StandaloneView::class);
        $view->setTemplateRootPaths(['EXT:ai_suite/Resources/Private/Templates']);
        ...
        $view->assignMultiple($assignMultiple);
        $view->setTemplatePathAndFilename('EXT:ai_suite/Resources/Private/Templates/' . $template . '.html');
        return $view->render();
    }
}

...
class SiteService implements SingletonInterface
{
        ...
        foreach ($this->siteFinder->getAllSites() as $site) {
            foreach ($site->getLanguages() as $language) {
                if($includeLanguageIds) {
                    $availableLanguages[$language->getTwoLetterIsoCode() . '__' . $language->getLanguageId()] = $language->getTitle();
                } else {
    ...
    protected function addSiteLanguagesToConsolidatedList(array $allSystemLanguages, array $languagesOfSpecificSite): array
    {
        foreach ($languagesOfSpecificSite as $language) {
            ...
            if (isset($allSystemLanguages[$languageId])) {
                $allSystemLanguages[$languageId]['isoCode'] = $language->getTwoLetterIsoCode();
            } else {
     ...
     * @throws SiteNotFoundException
     */
    public function getIdOfFirstRootPage() {
        $sites = $this->siteFinder->getAllSites();
        if (count($sites) > 0) {
            $firstSite = reset($sites);
            return $firstSite->getRootPageId();
        }
        throw new SiteNotFoundException('No site configurations found', 1521716623);
    }
}

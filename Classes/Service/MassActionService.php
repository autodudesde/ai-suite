    ...
    public function filelistFileDirectorySupport(ClientAnswer $librariesAnswer): array
    {
        ...
        if ($directoryId !== '') {
            ...
            if (count($files) > 0) {
                ...
                $languageParts = isset($sessionData['massActionFilesPrepare']['sysLanguage']) ? explode('__', $sessionData['massActionFilesPrepare']['sysLanguage']) : [];
                $column = $sessionData['massActionFilesPrepare']['column'] ?? 'all';
                $languageId = isset($languageParts[1]) ? (int)$languageParts[1] : 0;
                $showOnlyEmpty = isset($sessionData['massActionFilesPrepare']['showOnlyEmpty']) && $sessionData['massActionFilesPrepare']['showOnlyEmpty'] === 'true';
                $showOnlyUsed = isset($sessionData['massActionFilesPrepare']['showOnlyUsed']) && $sessionData['massActionFilesPrepare']['showOnlyUsed'] === 'true';
        ...
        return [
            ...
            'activeColumn' => $sessionData['massActionFilesPrepare']['column'] ?? 'all',
            ...
            'preSelection' => $sessionData['massActionFilesPrepare'] ?? [],
